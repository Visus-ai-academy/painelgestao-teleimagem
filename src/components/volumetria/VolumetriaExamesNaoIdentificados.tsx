...
export function VolumetriaExamesNaoIdentificados() {
  ...
  const loadExamesNaoIdentificados = async () => {
    try {
      ...
      console.log(
        "📊 Estudos únicos encontrados:",
        new Set(volumetriaData?.map((item) => item.ESTUDO_DESCRICAO).filter(Boolean)).size,
      );
      ...
      console.log(
        "  📋 Lista dos que estão no De Para:",
        noDePara.map((e) => `${e.estudo_descricao} (${e.quantidade})`),
      );
      console.log("  🔧 Exames que estão nas Regras de Quebra:", nasRegras.length);
      console.log(
        "  🔧 Lista dos que estão nas regras:",
        nasRegras.map((e) => `${e.estudo_descricao} (${e.quantidade})`),
      );
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-sm text-muted-foreground mt-1">
              Total de {totalExamesNaoIdentificados} exames sem quantidade de {examesNaoIdentificados.length} tipos
              únicos (US excluído)
            </div>
          </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-blue-800 dark:text-blue-200">
              ...
              <ul className="list-disc ml-4 mt-2 space-y-1">
                <li>
                  <strong>"Na tabela De Para":</strong> Exames cadastrados que precisam ser corrigidos. Use o botão{" "}
                  <strong>"Corrigir Exames Fora do Padrão"</strong> na aba Sistema de Regras para aplicar as correções
                  automaticamente.
                </li>
                <li>
                  <strong>"Não identificados":</strong> Exames que ainda não foram cadastrados. Adicione-os no cadastro
                  de exames primeiro, depois execute a correção.
                </li>
              </ul>
            </div>
          </div>
