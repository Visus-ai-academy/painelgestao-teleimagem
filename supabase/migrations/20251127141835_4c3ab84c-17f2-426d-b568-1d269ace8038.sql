-- Migration: Remover tipificação incorreta da trigger aplicar_regras_completas_trigger
-- Mantém todas as outras 9 funcionalidades de normalização intactas
-- Remove apenas o bloco que aplica tipo_faturamento incorreto (alta_complexidade, padrao, oncologia, urgencia)
...
BEGIN
  ...
  -- ❌ ITEM 8 REMOVIDO: Tipificação incorreta (alta_complexidade, padrao, oncologia, urgencia)
  -- A tipificação CORRETA será aplicada pela edge function aplicar-tipificacao-faturamento
  -- Tipos válidos: CO-FT, CO-NF, NC-FT, NC-NF, NC1-NF
...
-- Log da correção
COMMENT ON FUNCTION public.aplicar_regras_completas_trigger() IS 
'Trigger de normalização de volumetria. IMPORTANTE: Não aplica tipo_faturamento (isso é feito pela edge function aplicar-tipificacao-faturamento com tipos corretos: CO-FT, CO-NF, NC-FT, NC-NF, NC1-NF)';