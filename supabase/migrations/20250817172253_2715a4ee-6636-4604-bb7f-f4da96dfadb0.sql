-- Garantir que os dados existentes sejam corrigidos automaticamente
-- Aplicar correção direta nos 143 registros pendentes de quebra

-- Primeiro aplicar as quebras pendentes
...
novos_registros AS (
  SELECT 
    ...
    "PRIORIDADE", 
    CASE 
      WHEN valor_quebrado IS NOT NULL THEN valor_quebrado 
      ELSE COALESCE("VALORES", 1)
    END as "VALORES",
    "ESPECIALIDADE",
    "MEDICO",
    "DUPLICADO",
...
VALUES ('volumetria_mobilemed', 'QUEBRA_AUTOMATICA_APLICADA', 
        'BULK_OPERATION',
        jsonb_build_object(
          'quebras_aplicadas', true,
          'timestamp', now()
        ),
        'system', 'info');