-- Atualizar função para usar nome_fantasia como campo principal de correlação
CREATE OR REPLACE FUNCTION public.aplicar_validacao_cliente_volumetria(lote_upload_param text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    -- Buscar cliente correspondente usando NOME_FANTASIA como prioridade principal
    SELECT 
      c.id,
      c.nome_fantasia,
      c.tipo_cliente,
    ...
    WHERE c.nome_fantasia = registro_volumetria."EMPRESA"
       OR c.nome_mobilemed = registro_volumetria."EMPRESA"
  ...
  VALUES ('volumetria_mobilemed', 'VALIDACAO_CLIENTE', 
          ...
          jsonb_build_object(
            ...
            'campo_prioridade', 'nome_fantasia'
          ),
          'system', 'info');
  ...
  RETURN jsonb_build_object(
    ...
    'clientes_nao_encontrados', clientes_nao_encontrados,
    'campo_prioridade_usado', 'nome_fantasia',
