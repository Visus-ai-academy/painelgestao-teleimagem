-- Corrigir lógica de cálculo de franquia na RPC calcular_faturamento_completo
-- Quando frequencia_continua=false e frequencia_por_volume=false:
-- Se volume >= volume_franquia: NÃO cobra franquia (valor = 0)
-- Se volume < volume_franquia: cobra valor da franquia
...
BEGIN
  ...
  -- Calcular franquia (LÓGICA CORRIGIDA)
  IF v_parametros.param_aplicar_franquia THEN
    IF v_parametros.param_frequencia_continua THEN
      -- FREQUENCIA CONTÍNUA: cobra sempre, mas pode ter valor diferente se volume > volume_franquia
      ...
      END IF;
    ELSE
      -- SEM FREQUENCIA CONTÍNUA
      IF p_volume_total > 0 THEN
        IF v_parametros.param_frequencia_por_volume THEN
          -- COM FREQUENCIA POR VOLUME: cobra valor acima da franquia se volume > volume_franquia
          IF p_volume_total > COALESCE(v_parametros.param_volume_franquia, 0) THEN
            v_valor_franquia := COALESCE(v_parametros.param_valor_acima_franquia, v_parametros.param_valor_franquia, 0);
            v_detalhes_franquia := jsonb_build_object(
              'tipo', 'volume_acima',
              'volume_base', v_parametros.param_volume_franquia,
          ...
          ELSE
            -- Volume <= volume_franquia: cobra valor da franquia
            v_valor_franquia := COALESCE(v_parametros.param_valor_franquia, 0);
            v_detalhes_franquia := jsonb_build_object(
              'tipo', 'volume_normal',
              'volume_atual', p_volume_total,
          ...
          END IF;
        ELSE
          -- SEM FREQUENCIA POR VOLUME: só cobra se volume < volume_franquia
          IF p_volume_total >= COALESCE(v_parametros.param_volume_franquia, 0) AND v_parametros.param_volume_franquia > 0 THEN
            -- Volume >= volume_franquia: NÃO cobra franquia
            v_valor_franquia := 0;
            v_detalhes_franquia := jsonb_build_object(
              'tipo', 'isento_por_volume',
              'volume_base', v_parametros.param_volume_franquia,
              'volume_atual', p_volume_total,
              'valor_aplicado', 0,
              'motivo', 'Volume atingiu/superou meta - franquia não aplicada',
              'fonte', v_fonte_parametros
            );
          ELSE
            -- Volume < volume_franquia: cobra valor da franquia
            v_valor_franquia := COALESCE(v_parametros.param_valor_franquia, 0);
            v_detalhes_franquia := jsonb_build_object(
              'tipo', 'volume_abaixo_meta',
              'volume_base', v_parametros.param_volume_franquia,
              'volume_atual', p_volume_total,
              'valor_aplicado', v_valor_franquia,
              'motivo', 'Volume abaixo da meta - franquia aplicada',
              'fonte', v_fonte_parametros
            );
          END IF;
        END IF;
