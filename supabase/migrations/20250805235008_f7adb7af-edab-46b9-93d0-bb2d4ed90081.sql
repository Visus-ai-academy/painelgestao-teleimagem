-- Corrigir função get_volumetria_complete_data para remover limite de 1000 registros
...
AS $function$
BEGIN
  -- FORÇAR bypass completo de RLS e REMOVER qualquer limitação
  SET LOCAL row_security = off;
  SET LOCAL statement_timeout = 0;
  
  -- Retornar TODOS os dados sem LIMIT ou paginação
  RETURN QUERY
  SELECT 
    vm."EMPRESA",
    vm."MODALIDADE", 
...
-- Corrigir função get_volumetria_dashboard_stats para calcular atrasos baseado em VALORES
DROP FUNCTION IF EXISTS public.get_volumetria_dashboard_stats();
...
BEGIN
  ...
  -- Calcular estatísticas agregadas - CORRIGIDO: atrasos baseados em VALORES
  SELECT 
...
GRANT EXECUTE ON FUNCTION public.get_volumetria_dashboard_stats() TO authenticated;