-- ============= IMPLEMENTAÇÃO DE PARTICIONAMENTO (CORRIGIDO) =============
...
BEGIN
    ...
    IF table_exists THEN
        RETURN TRUE; -- Partição já existe
    END IF;
    ...
    -- Criar partição para tabela futura (quando implementar particionamento)
    -- Por ora, apenas logamos a intenção
    INSERT INTO audit_logs (table_name, operation, record_id, new_data, user_email, severity)
    VALUES ('volumetria_mobilemed', 'PARTITION_PLANNED', partition_name, 
            jsonb_build_object(
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_name = 'volumetria_mobilemed'
    ) INTO table_exists;
    
    IF table_exists THEN
        -- Log que a tabela existe e está pronta para particionamento futuro
        INSERT INTO audit_logs (table_name, operation, record_id, new_data, user_email, severity)
        VALUES ('volumetria_mobilemed', 'PARTITION_ASSESSMENT', 'main_table',
                jsonb_build_object(
                    'status', 'table_exists',
                    'partition_strategy', 'monthly_by_data_referencia',
                    'recommendation', 'implement_when_volume_increases'
                ),
                'system', 'info');
    ELSE
        -- Tabela não existe, será criada com particionamento quando necessário
        INSERT INTO audit_logs (table_name, operation, record_id, new_data, user_email, severity)
        VALUES ('volumetria_mobilemed', 'PARTITION_ASSESSMENT', 'main_table',
                jsonb_build_object(
                    'status', 'table_not_found',
                    'action', 'will_create_partitioned_when_needed'
                ),
                'system', 'warning');
    END IF;
END;
...
('volumetria_total_exames', 
 ...
 'SELECT COALESCE(SUM("VALORES"), 0) as value',
 'gauge',
 '{"period": "30_days"}'
),
...
-- ============= FUNÇÕES DE GESTÃO DE PARTIÇÕES =============
...
BEGIN
    ...
    -- Log da análise
    INSERT INTO audit_logs (table_name, operation, record_id, new_data, user_email, severity)
    VALUES ('volumetria_mobilemed', 'PARTITION_ANALYSIS', 'analysis',
            analysis, 'system', 'info');
...
BEGIN
    ...
    -- Log da atualização
    INSERT INTO audit_logs (table_name, operation, record_id, new_data, user_email, severity)
    VALUES ('custom_metrics', 'BULK_UPDATE', 'metrics_update',
            jsonb_build_object('updated_metrics', updated_count, 'timestamp', NOW()),
            'system', 'info');
