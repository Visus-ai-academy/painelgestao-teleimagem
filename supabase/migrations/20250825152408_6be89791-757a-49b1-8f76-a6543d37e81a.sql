-- Corrigir a regra v031 para ser um filtro de período válido, não apenas um limite superior
...
BEGIN
  ...
  -- REGRA v003: DATA_REALIZACAO não pode ser futura (>= 01/01/2025)
  IF record."DATA_REALIZACAO" >= '2025-01-01' THEN
    INSERT INTO registros_rejeitados_processamento (
      arquivo_fonte, lote_upload, linha_original, dados_originais, 
      motivo_rejeicao, detalhes_erro
    ) VALUES (
      ...
      'DATA_REALIZACAO futura: ' || record."DATA_REALIZACAO"::TEXT || ' >= 2025-01-01'
    );
    RETURN NULL;
  END IF;
