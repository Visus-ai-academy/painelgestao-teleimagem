-- Corrigir função limpar_dados_ficticios com sintaxe EXECUTE correta
...
BEGIN
  ...
  -- Verificar se coluna created_by existe na tabela medicos
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'medicos' AND column_name = 'created_by'
  ) INTO v_has_created_by;

  IF v_has_created_by THEN
    -- Remover médicos fictícios com created_by
    DELETE FROM medicos 
    WHERE created_by IS NOT NULL
    AND NOT EXISTS (
      SELECT 1 FROM import_history ih 
  ...
  ELSE
    -- Remover médicos que não estão na volumetria
    DELETE FROM medicos 
    WHERE NOT EXISTS (
      SELECT 1 FROM volumetria_mobilemed vm 
      WHERE vm."MEDICO" = medicos.nome
