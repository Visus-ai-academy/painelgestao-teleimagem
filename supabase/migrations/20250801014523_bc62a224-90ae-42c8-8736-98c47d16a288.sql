-- Sistema de Controle de Presença em Tempo Real
CREATE TABLE public.presenca_medico (
  ...
  checkout_automatico BOOLEAN DEFAULT false,
  status_presenca TEXT NOT NULL DEFAULT 'ausente' CHECK (status_presenca IN ('presente', 'ausente', 'checkout_manual', 'checkout_automatico')),
  ip_address INET,
  ...
  FOREIGN KEY (escala_id) REFERENCES escalas_medicas(id) ON DELETE CASCADE
);

-- Enable RLS
ALTER TABLE public.presenca_medico ENABLE ROW LEVEL SECURITY;

-- RLS Policies para presença médico
CREATE POLICY "Médicos podem gerenciar sua própria presença" 
ON public.presenca_medico 
FOR ALL 
...
CREATE POLICY "Admins e managers podem ver todas presenças" 
ON public.presenca_medico 
FOR SELECT 
USING (is_manager_or_admin());

CREATE POLICY "Admins podem gerenciar todas presenças" 
ON public.presenca_medico 
FOR ALL 
USING (is_admin())
WITH CHECK (is_admin());
...
-- Função para checkout automático (será chamada por cron/scheduler)
...
CREATE TRIGGER update_presenca_medico_updated_at
BEFORE UPDATE ON public.presenca_medico
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

-- Habilitar realtime para presença
ALTER TABLE public.presenca_medico REPLICA IDENTITY FULL;
ALTER PUBLICATION supabase_realtime ADD TABLE public.presenca_medico;

-- Comentários das tabelas
COMMENT ON TABLE public.presenca_medico IS 'Controla a presença em tempo real dos médicos durante seus turnos';
COMMENT ON TABLE public.logs_presenca IS 'Log de auditoria de todas as ações de presença dos médicos';