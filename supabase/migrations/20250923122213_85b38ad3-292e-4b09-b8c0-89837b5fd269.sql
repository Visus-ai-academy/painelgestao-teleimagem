-- Corrigir a função para usar cond_volume do contrato
...
DECLARE
  ...
  detalhes JSONB := '{}';
BEGIN
  -- Buscar informações do contrato para o cond_volume
  SELECT cc.* INTO contrato_info
  ...
  -- Se não encontrou contrato, usar padrão MOD/ESP/CAT
  IF NOT FOUND THEN
    contrato_info.cond_volume := 'MOD/ESP/CAT';
    contrato_info.considera_plantao := false;
  END IF;
  ...
  -- Aplicar lógica de faixas de volume do contrato se existir
  IF contrato_info.faixas_volume IS NOT NULL AND jsonb_array_length(contrato_info.faixas_volume) > 0 THEN
    -- Usar faixas de volume do contrato
    DECLARE
      faixa JSONB;
    BEGIN
      FOR faixa IN SELECT * FROM jsonb_array_elements(contrato_info.faixas_volume)
      LOOP
        IF volume_para_calculo >= (faixa->>'volume_inicial')::INTEGER 
           AND volume_para_calculo <= (faixa->>'volume_final')::INTEGER THEN
          -- Aplicar o preço base da linha de preço com possível ajuste de plantão
          valor_final := CASE 
            WHEN p_is_plantao AND contrato_info.considera_plantao AND preco_linha.valor_urgencia IS NOT NULL 
              THEN preco_linha.valor_urgencia
    ...
    END;
  ELSE
    -- Usar valores base da linha de preço se não há faixas específicas
  ...
  RETURN QUERY SELECT 
    ...
    faixa_usada,
    volume_para_calculo,
    COALESCE(contrato_info.cond_volume, 'MOD/ESP/CAT'),
    detalhes;
END;
$$;