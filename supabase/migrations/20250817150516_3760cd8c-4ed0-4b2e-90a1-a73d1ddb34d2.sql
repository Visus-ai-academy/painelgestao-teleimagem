-- Remover o trigger problemático atual
DROP TRIGGER IF EXISTS trigger_quebra_exames_insert ON volumetria_mobilemed;
DROP FUNCTION IF EXISTS trigger_aplicar_quebra_exames();

-- Criar função otimizada que marca registros para quebra posterior
CREATE OR REPLACE FUNCTION trigger_marcar_para_quebra()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  -- Apenas marcar registros que precisam de quebra (sem fazer a quebra no trigger)
  IF EXISTS (
    SELECT 1 FROM regras_quebra_exames 
    WHERE exame_original = NEW."ESTUDO_DESCRICAO" AND ativo = true
  ) THEN
    -- Marcar no campo para processamento posterior
...
-- Adicionar coluna para marcar processamento pendente (se não existir)
ALTER TABLE volumetria_mobilemed 
ADD COLUMN IF NOT EXISTS processamento_pendente BOOLEAN DEFAULT false;

-- Criar trigger otimizado (apenas marca, não processa)
CREATE TRIGGER trigger_marcar_quebra_pendente
  BEFORE INSERT ON volumetria_mobilemed
  FOR EACH ROW
  EXECUTE FUNCTION trigger_marcar_para_quebra();

-- Atualizar a função existente para processar registros marcados
CREATE OR REPLACE FUNCTION aplicar_regras_quebra_exames(arquivo_fonte_param text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FOR registro IN 
    ...
    AND (vm.processamento_pendente = true OR vm.processamento_pendente IS NULL)
    GROUP BY vm.id, vm."EMPRESA", vm."NOME_PACIENTE", vm."CODIGO_PACIENTE", 
             ...
             vm.updated_at, vm.tipo_faturamento, vm.processamento_pendente
  LOOP
    ...
    -- Deletar registro original e marcar como processado
