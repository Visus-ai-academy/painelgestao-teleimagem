-- Atualizar constraint para incluir as operações de TRUNCATE
...
CHECK (operation = ANY (ARRAY[
  ...
  'LIMPEZA_DADOS'::text, 
  'LIMPEZA_COMPLETA'::text,
  'TRUNCATE_TABLE'::text,      -- Novo valor adicionado
  'TRUNCATE_COMPLETED'::text,  -- Novo valor adicionado
  'FECHAR_PERIODO'::text, 
