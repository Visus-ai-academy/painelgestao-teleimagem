...
ALTER TABLE IF EXISTS clientes 
  ADD CONSTRAINT IF NOT EXISTS chk_clientes_status CHECK (status IN ('Ativo', 'Inativo', 'Suspenso'));

ALTER TABLE IF EXISTS faturamento
  ADD CONSTRAINT IF NOT EXISTS chk_faturamento_valor_positivo CHECK (valor > 0);

-- Constraint para email único em clientes
ALTER TABLE IF EXISTS clientes 
  ADD CONSTRAINT IF NOT EXISTS unq_clientes_email UNIQUE (email);
