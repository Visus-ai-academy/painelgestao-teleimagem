-- Primeiro, remover as constraints que dependem da tabela omie_faturas
ALTER TABLE public.regua_cobranca DROP CONSTRAINT IF EXISTS regua_cobranca_fatura_id_fkey;
ALTER TABLE public.emails_cobranca DROP CONSTRAINT IF EXISTS emails_cobranca_fatura_id_fkey;

-- Excluir a tabela omie_faturas
DROP TABLE IF EXISTS public.omie_faturas CASCADE;
...
-- Recriar as constraints apontando para a nova tabela
ALTER TABLE public.regua_cobranca 
ADD CONSTRAINT regua_cobranca_fatura_id_fkey 
FOREIGN KEY (fatura_id) REFERENCES public.faturamento(id);

ALTER TABLE public.emails_cobranca 
ADD CONSTRAINT emails_cobranca_fatura_id_fkey 
FOREIGN KEY (fatura_id) REFERENCES public.faturamento(id);