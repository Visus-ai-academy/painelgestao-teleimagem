-- Corrigir e simplificar aplicação de quebras
CREATE OR REPLACE FUNCTION public.aplicar_quebras_pendentes_corrigido()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- Usar valor padrão do registro original
      valor_quebrado := COALESCE(registro."VALORES", 1);
