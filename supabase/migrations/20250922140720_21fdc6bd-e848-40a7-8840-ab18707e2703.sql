-- Corrigir a função calcular_preco_exame para usar SEMPRE o valor_base da linha encontrada
-- (sem fallback e sem trocar para valor_urgencia quando prioridade = URGÊNCIA/PLANTÃO)
...
BEGIN
  ...
  -- Se não encontrou preço, retornar erro (sem fallback)
  IF NOT FOUND THEN
    RETURN QUERY SELECT 
  ...
  IF v_cond_volume IS NOT NULL AND v_cond_volume != '' THEN
    ...
    -- Rebuscar o preço correto baseado no volume calculado (mantendo prioridade específica)
    SELECT ps.*
    INTO v_preco
    FROM precos_servicos ps
    WHERE ps.cliente_id = p_cliente_id
  ...
  -- Retornar SEMPRE o valor_base da linha encontrada (sem fallback)
  RETURN QUERY SELECT 
    ...
    v_detalhes || jsonb_build_object('campo_utilizado', 'valor_base') as detalhes_calculo;
END;
$function$;