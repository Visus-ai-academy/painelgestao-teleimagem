...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const arquivoAtual of arquivos) {
      ...
      // ⚠️ REGRAS v002/v003: DESATIVADAS - NÃO APLICAR AUTOMATICAMENTE
      // As regras v002/v003 NÃO devem ser aplicadas automaticamente porque excluem 
      // registros baseados no período selecionado, mas a lógica precisa ser aplicada 
      // MANUALMENTE após análise correta do período de faturamento vs período dos dados.
      // 
      // Para aplicar manualmente, o usuário deve usar:
      // - Menu "Sistema de Regras" → "Correção v002/v003"
      // - Ou chamar a edge function aplicar-exclusoes-periodo diretamente
      if (arquivoAtual.includes('retroativo')) {
        console.log('  ⚠️ v002/v003 - DESATIVADAS (não aplicadas automaticamente)')
        console.log('    📝 Arquivo retroativo detectado, mas regras v002/v003 NÃO serão aplicadas')
        console.log('    📝 Use "Sistema de Regras" → "Correção v002/v003" para aplicação manual')
